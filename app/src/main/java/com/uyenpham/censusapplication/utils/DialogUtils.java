...
//  private static AlertDialog sErrorAlert;
    ...
    public static synchronized void showProgressDialog(final Activity activity) {
        try {
            ...
            sProgress = ProgressDialog.show(activity, "", activity.getString(R.string.txt_loading),
                    true, false);
        } catch (Exception e) {
            Logger.e(e);
        }
    ...
    public static synchronized void showProgressDialog(final Context activity) {
        try {
            dismissProgressDialog();
            if (!isValidContext(activity)) {
                return;
    ...
    public static synchronized void showProgressDialog(final Context activity, String content) {
        try {
            dismissProgressDialog();
            if (!isValidContext(activity)) {
                return;
            ...
            sProgress = ProgressDialog.show(activity, "", content,
                    true, false);
        } catch (Exception e) {
            Logger.e(e);
        }
...
//  }
     ...
     * Show alert dialog with action
     */
    public static void showAlertNotiAction(Context context, String message, DialogInterface.OnClickListener listener) {
        if (null == message || message.trim().isEmpty()) return;
...
//    dismissAlert();
        ...
        // Show dialog
        showDialogMessage(activity, activity.getString(R.string.title_error),
    ...
    public interface CustomDialogCallback {
        void onClick(Button btnSkip);
    }

    public static boolean isShowingProcessDialog() {
        if (sProgress != null && sProgress.isShowing()) return true;
        return false;
    }
}
